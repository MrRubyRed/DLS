...
class HPolytope(object):
    ...
    def find_all_dynamic_neighbors(self,policy,dynamics,pW,dW,dt):
        for i in range(len(self.exit_facets)):
            if self.exit_facets[i]:
                tmp_point = self.points[i]
                while np.matmul(self.A[i,:],tmp_point) <= self.b[i]:
                    tmp_point += 1e-6*self.A[i,:]
                tmp_neighbor = self.check_if_in_union_BFS(tmp_point)
                if tmp_neighbor is None:
                    self.neighbors.append(self.get_polytope(tmp_point,policy,dynamics,pW,dW,dt))
                    self.neighbors[-1].neighbors.append(self)
                    self.neighbors[-1].find_all_dynamic_neighbors(policy,dynamics,pW,dW,dt)
                else:
                    self.neighbors.append(tmp_neighbor)
...
def create_polytope_tree(state,policy,dynamics,pW,dW,dt):
    list_Wp,list_bp = pW                            #Unpack weightd and biases for the policy
    Ap,bp = get_region(list_Wp,list_bp,state)       #Get H-representation Polytope for state
    K = policy.get_Jacobian(state)                  #Get K (from u = Kx + d)
    d = policy.act(False,state) - np.matmul(K,state)#Get d (from u = Kx + d)
    ...
    state_action = np.concatenate((state,action),axis=0)    #Concatenate state and action
    list_Wd,list_bd = dW                                    #Unpack weights and biases for the dynamics
    Ad,bd = get_region(list_Wd,list_bd,state_action)        #Get H-representation Polytope for state-action
    ...
    new_A = np.concatenate((Ap,Ap_),axis=0)
    new_b = np.concatenate((bp,bp_),axis=0) 
    A,b = hyperP_filter(new_A,new_b)
    
    polytope = HPolytope(A,b,dyn_Af,dyn_c)
